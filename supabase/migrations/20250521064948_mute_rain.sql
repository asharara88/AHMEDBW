/*
  ...
  3. Views
    - `stripe_user_subscriptions`: User-specific subscription details
    - `stripe_user_orders`: User-specific order details
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can view their own customer data' AND tablename = 'stripe_customers') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can view their own subscription data' AND tablename = 'stripe_subscriptions') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can view their own order data' AND tablename = 'stripe_orders') THEN
